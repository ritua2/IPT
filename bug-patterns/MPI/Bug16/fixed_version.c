...
void barrier(){    
    ...
    if(my_rank == 0){
        ...
        *(nodes_barrier)  = true;
        ...
        while(start_count < num_procs){ 
            MPI_Recv(&b_req,1,MPI_CHAR, MPI_ANY_SOURCE, 1, MPI_COMM_WORLD, &status);            
        ...
        for(j=1; j<num_procs; j++)
            MPI_Send(&b_req,1,MPI_CHAR,j,2,MPI_COMM_WORLD);
        ...
        while(end_count < num_procs){ 
            MPI_Recv((double *)tmp,2,MPI_DOUBLE, MPI_ANY_SOURCE, 3, MPI_COMM_WORLD, &status);            
    ...
    //**Barier implementation for Pn-1 (except P0)**
        ...
        *(execution + 0) = MPI_Wtime();
        ...
        MPI_Send(&b_req,1,MPI_CHAR,0,1,MPI_COMM_WORLD);
        //Receive command from P0 that it can release its barrier
        MPI_Recv(&b_req,1,MPI_CHAR, 0, 2, MPI_COMM_WORLD, &status);
        //Measure and send execution time to P0   
        *(execution + 1) = MPI_Wtime();
        MPI_Send((double *)execution,2,MPI_DOUBLE,0,3,MPI_COMM_WORLD); 
        free(execution);
    }    
}
